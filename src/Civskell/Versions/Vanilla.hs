  ...
  {packetsForState = \case
    Handshaking -> Vector.fromList 
      [ambiguate $ Server.handshake $ \(Server.Handshake protocol _addr _port newstate) -> protocolVersion <$> ask >>= \prot -> if fromIntegral protocol == prot
        ...
        then case newstate of
          1 -> modify $ playerState .~ Status
          2 -> modify $ playerState .~ LoggingIn
          ...
          Right ss -> do
            ...
            serverAuthentication name loginHash >>= \case
              ...
              Just (AuthPacket uuid nameFromAuth authProps) -> do
                ...
                c <- ask
                pid <- _playerId <$> get
                modify $ playerUsername .~ nameFromAuth
                modify $ playerClientUUID .~ uuid
                ...
                modify $ playerGamemode .~ defaultGamemode c
                ...
                ps <- Map.elems . view worldPlayers <$> get
    ...
    Status -> Vector.fromList 
      [ambiguate $ Server.statusRequest $ \_ -> do
        ...
        playersOnline <- Map.elems . view worldPlayers <$> get
        motd <- serverMotd <$> ask
    ...
    Playing -> Vector.fromList
      ...
      ,ambiguate $ Server.chatMessage $ \(Server.ChatMessage (ProtocolString msg)) -> case msg of
        "/gamemode 1" -> modify $ playerGamemode .~ Creative
        "/gamemode 0" -> modify $ playerGamemode .~ Survival
      ...
      ,ambiguate $ Server.clientSettings $ \(Server.ClientSettings _loc _viewDist _chatMode _chatColors _skin _hand) -> do
        ...
        modify $ \p -> (playerTPConfirmQueue %~ Set.insert (view playerNextTid p)) . (playerNextTid %~ succ) $ p 
        tid <- view playerNextTid <$> get
      ...
      ,ambiguate $ Server.confirmTransaction $ \(Server.ConfirmTransaction wid transId _acc) -> Set.member (wid,transId) . view playerFailedTransactions <$> get >>= \case
        True -> do
          -- Log when they aplogize
          logg $ "Client apologized for bad transaction: " <> showText (wid,transId)
          -- Remove the offending transaction
          modify $ playerFailedTransactions %~ Set.delete (wid,transId)
      ...
      ,ambiguate $ Server.clickWindow $ \(Server.ClickWindow wid slotNum transId mode _clientProvidedSlot) -> do
        ...
        view (playerWindows . at wid) <$> get >>= \case
          ...
          Just (SuchThat (Window desc w)) -> Set.null . view playerFailedTransactions <$> get >>= \case
      ...
      ,ambiguate $ Server.closeWindow $ \(Server.CloseWindow w) -> do
        ...
        case w of
          0 -> pure ()
          wid -> modify $ playerWindows . at wid .~ Nothing
      -- BS.tail removes the length prefixing
      ,ambiguate $ Server.pluginMessage $ \case
        (Server.PluginMessage "MC|Brand" cliBrand) -> modify $ playerClientBrand .~ show (BS.tail cliBrand)
        (Server.PluginMessage chan msg) -> logp $ "Unsupported Plugin Message! " <> showText chan <> " | " <> showText msg
      ,ambiguate $ Server.useEntity $ \(Server.UseEntity targetEID _action) -> view (worldEntities . at targetEID) <$> get >>= \case
      ...
      ,ambiguate $ Server.playerPosition $ \(Server.PlayerPosition (x,y,z) _grounded) -> modify $ playerPosition .~ (x,y,z)
      ,ambiguate $ Server.playerPositionAndLook $ \(Server.PlayerPositionAndLook (x,y,z) (yaw,pitch) _grounded) -> modify $ (playerPosition .~ (x,y,z)) . (playerViewAngle .~ (yaw,pitch))
      ,ambiguate $ Server.playerLook $ \(Server.PlayerLook (y,p) _grounded) -> modify $ playerViewAngle .~ (y,p)
      ...
      ,ambiguate $ Server.playerAbilities $ \(Server.PlayerAbilities (AbilityFlags _i f _af _c) _flySpeed _fovMod) -> modify $ playerMoveMode .~ if f then Flying else Walking
      ,ambiguate $ Server.playerDigging $ \(Server.PlayerDigging action) -> case action of
        StartDig block _side -> do
          ...
          -- Instant Dig
          modify $ blockInWorld block .~ ambiguate (Block air Air)
        SwapHands -> do
          -- Get the current items
          heldSlot <- view playerHoldingSlot <$> get
        ...
        DropItem isStack -> do
          ...
          case heldItem of
            ...
            SuchThat (Slot (Just heldData)) -> do
              ...
              plaLoc <- view playerPosition <$> get
      ...
      ,ambiguate $ Server.entityAction $ \(Server.EntityAction _eid action) -> case action of
        Sneak True -> modify $ playerMoveMode .~ Sneaking
        Sneak False -> modify $ playerMoveMode .~ Walking
        Sprint True -> modify $ playerMoveMode .~ Sprinting
        Sprint False -> modify $ playerMoveMode .~ Walking
      ...
      ,ambiguate $ Server.heldItemChange $ \(Server.HeldItemChange slotNum) -> modify $ playerHoldingSlot .~ slotNum
      ...
      ,ambiguate $ Server.playerBlockPlacement $ \(Server.PlayerBlockPlacement block side hand cursorCoord) -> do
        ...
        SuchThat (Block desc b) <- view (blockInWorld block) <$> get
        case onClick desc of
          Just cb -> cb b block side hand cursorCoord
          Nothing -> do
            -- Find out what item they are trying to place
            heldSlot <- if hand == MainHand then view playerHoldingSlot <$> get else pure 45
            ...
            case theItem of
              ...
              SuchThat (Slot (Just theSlotData@(SlotData (Item idesc i) _cnt))) -> case onItemUse idesc of
                ...
                Just (dItem,oiu)-> do
                  ...
                  modify $ playerInventory . at actualSlot .~ Just (ambiguate $ dItem theSlotData)
      ...
      ,ambiguate $ Server.useItem $ \(Server.UseItem hand) -> do
        ...
        held <- getInventorySlot =<< (case hand of {MainHand -> view playerHoldingSlot <$> get; OffHand -> pure 45})
